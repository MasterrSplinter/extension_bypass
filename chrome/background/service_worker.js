...
async function fetchAndUpdateRules() {
  console.log('[WebflixBlocker/SW] 🔄 Tentative MàJ règles distantes...');
  try {
    ...
    console.log(`[WebflixBlocker/SW] ✅ ${numberedRules.length} règles chargées`);
  } catch (err) {
    console.warn('[WebflixBlocker/SW] ⚠️ Règles distantes non disponibles:', err.message);
  }
}
...
chrome.webNavigation.onCreatedNavigationTarget.addListener(async (details) => {
  ...
  if (isAdHostname(hostname)) {
    console.log(`[WebflixBlocker/SW] 🚫 Pub connue: ${hostname}`);
  ...
  if (details.sourceTabId) {
    try {
      ...
      if (fromStreaming || fromPlayer) {
        console.log(`[WebflixBlocker/SW] 🚫 Popup suspect depuis ${sourceHost} → ${hostname}`);
        if (await safeCloseTab(details.tabId)) {
          await incrementBlockedCount(hostname);
          notifyBlocked(hostname);
        }
  ...
  if (lastUserClickTime > 0) {
    ...
    if (timeSinceClick >= 0 && timeSinceClick < HEURISTIC_WINDOW_MS) {
      console.log(`[WebflixBlocker/SW] 🚫 Heuristique (${timeSinceClick}ms): ${hostname}`);
      if (await safeCloseTab(details.tabId)) {
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (isAdHostname(hostname)) {
    console.log(`[WebflixBlocker/SW] 🚫 Tab pub détecté: ${hostname}`);
  ...
  if (type === 'TOGGLE_PROTECTION') {
    ...
    chrome.storage.local.set({ enabled: newEnabled }).then(async () => {
      ...
      try {
        ...
        console.warn('[WebflixBlocker/SW] DNR toggle error:', e.message);
      }

      // Mettre à jour le badge
      await updateBadge();

      console.log(`[WebflixBlocker/SW] Protection ${newEnabled ? '✅ activée' : '⏸️ désactivée'}`);
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  await setupAlarms();
  console.log('[WebflixBlocker/SW] Extension installée/mise à jour');
});

// Badge initial
updateBadge();

console.log('[WebflixBlocker/SW] ✅ v1.5 démarré — Architecture robuste');
